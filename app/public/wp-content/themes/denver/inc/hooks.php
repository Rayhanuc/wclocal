...
if ( ! function_exists( 'denver_add_site_info' ) ) {
	...
	function denver_add_site_info() {
		...
		$site_info = sprintf(
			...
			sprintf(
				/* translators:*/
				esc_html__( 'Proudly powered by %s', 'denver' ),
				'WordPress'
			),
			sprintf( // WPCS: XSS ok.
				/* translators:*/
				esc_html__( 'Theme: %1$s by %2$s.', 'denver' ),
				$the_theme->get( 'Name' ),
				'<a href="' . esc_url( __( 'http://denver.com', 'denver' ) ) . '">denver.com</a>'
			),
			sprintf( // WPCS: XSS ok.
				/* translators:*/
				esc_html__( 'Version: %1$s', 'denver' ),
